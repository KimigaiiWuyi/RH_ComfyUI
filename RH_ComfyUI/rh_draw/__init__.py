...
from ..utils.comfyui.wrapper import gen_image_by_img, gen_image_by_text, gen_edit_img_by_img
...
@sv_draw.on_command(("图生图",), block=True)
async def draw_img_by_img(bot: Bot, ev: Event):
    prompt = ev.text.strip()
    ...
    if not ev.image_id:
        return await bot.send("图生图需要在命令后面加入一张图片！")

    img = await RM.get(ev.image_id)

    if await RHBind.deduct_point(ev.user_id, ev.bot_id, 2):
        await bot.send("💪 积分充足！已扣除1点积分!\n✅ 正在生成图片，预计将等待1分钟...")
        image = await gen_image_by_img(prompt, img)
...
async def edit_img_by_img(bot: Bot, ev: Event):
    ...
    img_list = [await RM.get(image_id) for image_id in ev.image_id_list]

    if await RHBind.deduct_point(ev.user_id, ev.bot_id, 2):
        await bot.send("💪 积分充足！已扣除1点积分!\n✅ 正在生成图片，预计将等待1分钟...")
        image = await gen_edit_img_by_img(prompt, img_list)
