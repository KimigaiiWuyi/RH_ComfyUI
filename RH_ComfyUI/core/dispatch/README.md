...
| 文件 | 内容 |
|---|---|
| `dispatcher.py` | `dispatch(request, ctx)`:route → validate + ensure_channel_pin(不扣费)→ reserve → 并发闸 + 执行 → settle/refund → record_dispatch |
...
| 校验 | `ValidationError`(含未知/不可用的钉扎通道) | 不扣费(校验先于扣费) |
