...
| 类 | 能力面 |
|---|---|
| `IndexTTS2Model` | 自然中文 TTS;`reference_audio` 参考音频音色克隆(一等端口,画布可连音频节点);情绪指令 |

参考音频数据流(蓝图 §10):画布音频节点 → HTTP 请求 `reference_audio`
...
- 新 TTS 模型若支持音色克隆,声明 `supports_voice_clone = True` 并在
  `input_schema()` 暴露 `reference_audio` 端口,前端即自动出现连线口;
