...
| 费率 | 见 §15.2.4 | 480p/720p:**70 / 42**;1080p:**77 / 46** |
...
yuan   = round(tokens × 费率元/M / 1e6, 2)   # 对齐价表两位小数
points = yuan × 100                          # 1 元 = 100 积分,最小 1
```

4s 参考视频 + 生成 4s 成片 = **8s token × 有输入单价**,不是「输出被算成 8 秒」。
无输入 4s 仍按 4s × 无输入单价。
...
写入 token 公式前,2.0/2.5 **有输入时把输入秒数抬到至少 4s**(价表「最低价对应输入 2~4 秒」)。
`duration=-1` 的**输出**时长仍跟真实输入,不抬。
...
```
_SEEDANCE25_RATES = {
  "480p"/"720p": (70.00 无输入, 42.00 有输入),
  "1080p": (77.00, 46.00),
}
480p 像素按 854×480(16:9),720p=1280×720,1080p=1920×1080,24fps。
```

| 场景 | 积分 |
|---|---|
| 720p 5s 无输入 | **756**(7.56 元) |
| 720p 5s + 输入 2~4s | **816**(8.16 元) |
| 720p 5s + 输入 5s | **907**(9.07 元) |
| 720p 5s + 输入 15s | **1814**(18.14 元) |
| 720p 5s + 输入 30s | **3175**(31.75 元) |
| 720p 4s + 输入 4s | **726**(7.26 元,8s token) |
| 480p 5s 无输入 | **336**(3.36 元) |
| 1080p 5s 无输入 | **1871**(18.71 元) |
...
# 关键断言
# - 720p 5s 无输入 = 756
# - input_video_duration=2 与 4 同价 → 816
# - input_video_duration=15 → 1814
# - 3 段无时长 = 15s 输入
