...
条件 import 或企业逻辑;接入面只有 `RH_ComfyUI.core` 顶层公开接口。
...
```python
# 闭源插件 RH_Enterprise/__init__.py
from gsuid_core.server import on_core_start

@on_core_start
async def register_enterprise_models() -> None:
    from RH_ComfyUI.core import model_registry
    from .models import EnterpriseSeedanceModel, InternalTTSModel

    model_registry.register(EnterpriseSeedanceModel())
...
enterprise = "rh_enterprise.models:provide_models"
```

```python
def provide_models() -> list[type]:
    return [EnterpriseSeedanceModel, InternalTTSModel]  # 类须可无参构造
```
...
企业插件用 `name = "seedance2"` 即可把开源 seedance2 换成走内部网关的
...
class EnterpriseSeedanceModel(VideoGenerationBase):
    name = "seedance2"
    display_name = "Seedance 2.0(内部)"
    modality = TaskType.VIDEO
    point_cost = 10
    card = ModelCard(description="走企业内部网关的 Seedance 2.0")
...
3. 企业独立钱包 → 实现自己的 `BillingPolicy` 子类,在**自己的入口**构造
...
5. 需要开源侧新钩子时,提交的是**通用**扩展点(新的 Policy/Channel 抽象),
   企业逻辑留在闭源包 —— 不接受 `if enterprise` 式提交。
