...
2. OC text 段本身不含结构化代号,Ark 侧仍只注入一次 [@参考图片N]
3. 扁平 banana/gpt-image 路径 ensure 为 no-op
4. happyhorse 能把 [@参考图片N] / [参考图片N] 改写为 [Image N]
"""
...
from RH_ComfyUI.utils.core.media_labels import ensure_media_ref_labels, append_spaced_media_ref
...
def _ark_like_text(spec) -> str:
    """模拟 Ark content[] 文本合并:segments 原文 + 媒体位注入 [@参考图片N]。"""
    parts: list[str] = []
    img = 0
    for s in spec.ordered_segments:
        if s.kind == "text" and s.text:
            ...
            img += 1
            append_spaced_media_ref(parts, "image", img)
    return "".join(parts)
...
def test_seedance_oc_hollow_prompt_rebuild_no_double_media():
    ...
    assert _ark_like_text(spec) == "将 [@参考图片1] 中的角色替换为 [@参考图片2] ，其他不变"
